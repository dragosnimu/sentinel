...
- **`/etc/sysconfig/suricata`** → `OPTIONS="--af-packet=<iface> -F <bpf> --set vars.address-groups.HOME_NET=[<ip-public>]"`.
...
- **drop-in `suricata.service.d/sentinel.conf`** → `MemoryMax=1G` (VPS mic).
