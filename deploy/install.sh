...
step_secrets() {
...

    {
        ...
        for key in ANTHROPIC_API_KEY TELEGRAM_BOT_TOKEN TELEGRAM_CHAT_ID \
                   TELEGRAM_CALLBACK_HMAC_KEY SENTINEL_DB_PASSWORD \
                   SENTINEL_SESSION_SECRET TELEGRAM_APPLY_PIN; do
            [[ -n "${SECRETS[$key]:-}" ]] && printf '%s=%s\n' "$key" "${SECRETS[$key]}"
        done
        # Generated locally rather than transferred: no reason for them to exist
        # off this host.
        grep -q '^TELEGRAM_CALLBACK_HMAC_KEY=' "${target}.tmp" 2>/dev/null || \
            printf 'TELEGRAM_CALLBACK_HMAC_KEY=%s\n' "$(openssl rand -hex 32)"
        grep -q '^SENTINEL_SESSION_SECRET=' "${target}.tmp" 2>/dev/null || \
            printf 'SENTINEL_SESSION_SECRET=%s\n' "$(openssl rand -hex 32)"
    } >> "${target}.tmp"
