...
# `-x` is true, and os.access(X_OK) is true for every existing file. So the
# assertions below deliberately do NOT claim anything about the mode — an
# os.access(X_OK) here passes whatever the step does, which is a test that
# checks nothing. What the mode is verified against instead: the shipped text
# (test_the_binary_is_installed_executable) here, and, on a real host, the
# step's own version probe, which cannot succeed on a non-executable file.
...
def test_the_binary_is_installed_executable():
    """A binary written 0644 is not installed, it is just present — and the
    dashboard would show a scanner that never produces a finding.

    This is an assertion on the shipped text, not on an effect, and that is a
    weaker thing. It is here because the effect is unobservable on the machine
    this suite runs on: measured on Windows/MSYS, `install -m 0644` still yields
    stat 755, so every runtime check of the mode passes regardless. On a real
    host the effect IS checked, twice, by the step itself: `[[ -x ]]` and then a
    version probe, which exits 126 on a file that cannot be executed. Mutating
    0755 to 0644 was the one change in this round that no runtime test caught;
    this line is what catches it, and it is honest about how.
    """
    body = step21_source()
